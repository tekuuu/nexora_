...
export class FhevmDecryptionSignature {
  ...
  static async loadFromLocalStorage(
    ...
    try {
      ...
      let result = localStorage.getItem(storageKey.key);

      if (!result) {
        // Legacy fallback: attempt to compute the old key using the previous mock instance
        try {
          const legacyKey = new FhevmDecryptionSignatureStorageKey(
            {
              createEIP712: () => ({
          ...
          result = localStorage.getItem(legacyKey.key);
        } catch {}
