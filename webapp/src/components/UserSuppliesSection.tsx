...
interface UserSuppliesSectionProps {
  ...
  // Whether collateral data has been loaded from smart contract
  isCollateralDataLoaded?: boolean;
}
...
const calculateUSDValue = (raw: bigint | null | undefined, decimals: number, price: number | undefined) => {
  ...
  // Price from hook is in USD (already scaled appropriately)
...
const UserSuppliesSection: React.FC<UserSuppliesSectionProps> = ({
  ...
  const positions = useMemo(() => {
    ...
    const mapped = entries.map(([symbol, bal]) => {
      ...
      return {
        ...
        isCollateralEnabled: isCollateralDataLoaded ? Boolean(userCollateralEnabled?.[symbol]) : Boolean(asset?.isCollateral),
    ...
    // sort by usdValue descending (treat null/NaN as 0)
    mapped.sort((a, b) => {
  ...
  const handleToggle = useCallback(async (asset: { address: string; symbol: string }, enabled: boolean) => {
    try {
      ...
      await onCollateralToggle(asset, enabled);
      ...
      setCollateralToggling(s => ({ ...s, [asset.symbol]: false }));
    }
  }, [onCollateralToggle]);
  ...
  const requestToggle = (asset: { address: string; symbol: string }, enabled: boolean) => {
    ...
    if (!enabled) {
      const hasBorrows = hasActiveBorrowsForSymbol ? hasActiveBorrowsForSymbol(asset.symbol) : false;
      if (hasBorrows) {
        setConfirmDialogAsset(asset);
        setConfirmDialogOpen(true);
        return;
      }
      // no borrows -> disable immediately
      handleToggle(asset, false);
      return;
    }
    // enabling immediately
    handleToggle(asset, true);
  };

  const confirmDisable = async () => {
    if (!confirmDialogAsset) return;
    setConfirmDialogOpen(false);
    await handleToggle(confirmDialogAsset, false);
  ...
  if (positions.length === 0) {
    return (
      <Box sx={{ textAlign: 'center', py: 6 }}>
        <Typography variant="h6" sx={{ mb: 1, color: isDarkMode ? 'white' : '#000000' }}>Supply assets to get started</Typography>
        <Typography variant="body2" sx={{ color: isDarkMode ? 'rgba(255, 255, 255, 0.7)' : 'rgba(0, 0, 0, 0.7)', mb: 2 }}>Supply assets to the protocol to earn interest and use as collateral</Typography>
  ...
  return (
    <Box>
      {errorMessage && (
        <Alert severity="error" onClose={() => setErrorMessage(null)} sx={{
          mb: 2,
          '& .MuiAlert-message': {
            ...
            {positions.map(p => (
              <TableRow key={p.symbol} hover>
                ...
                <TableCell align="center" sx={{ color: isDarkMode ? 'white' : '#000000' }}>
                  <Box>
                    <Typography sx={{ color: isDarkMode ? 'white' : '#000000' }}>{p.isDecrypted ? p.formattedSupplied : '••••••••'}</Typography>
                  </Box>
                </TableCell>
                  ...
                  <TableCell align="center" sx={{ color: isDarkMode ? 'white' : '#000000' }}>
                    {p.address.toLowerCase() === CONTRACTS.CONFIDENTIAL_WETH.toLowerCase() ? (
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, justifyContent: 'center' }}>
                        <Switch
                          key={p.symbol}
                          checked={isCollateralDataLoaded ? Boolean(userCollateralEnabled?.[p.address?.toLowerCase()] ?? userCollateralEnabled?.[p.symbol] ?? false) : p.isCollateralEnabled}
                          onChange={() => {
                            const current = isCollateralDataLoaded ? Boolean(userCollateralEnabled?.[p.address?.toLowerCase()] ?? userCollateralEnabled?.[p.symbol] ?? false) : p.isCollateralEnabled;
          ...
          <Card key={p.symbol} sx={{
            ...
            <CardContent>
              ...
              <Grid container spacing={1} sx={{ mt: 1 }}>
                <Grid item xs={6}>
                  <Typography variant="caption" sx={{ color: isDarkMode ? 'rgba(255, 255, 255, 0.7)' : 'rgba(0, 0, 0, 0.7)' }}>Balance</Typography>
                  <Typography sx={{ color: isDarkMode ? 'white' : '#000000' }}>{p.isDecrypted ? p.formattedSupplied : '••••••••'}</Typography>
                </Grid>
                <Grid item xs={6}>
                  <Typography variant="caption" sx={{ color: isDarkMode ? 'rgba(255, 255, 255, 0.7)' : 'rgba(0, 0, 0, 0.7)' }}>Value</Typography>
                  <Typography sx={{ color: isDarkMode ? 'white' : '#000000' }}>{formatUSDValue(p.usdValue === null ? null : p.usdValue, p.isDecrypted)}</Typography>
                </Grid>
                <Grid item xs={6}>
              ...
              <Box sx={{ mt: 2 }}>
                <Button fullWidth variant="outlined" onClick={() => onWithdrawClick({ address: p.address, symbol: p.symbol, decimals: p.decimals, name: p.name, icon: p.icon, color: p.color })} sx={{
                  borderColor: isDarkMode ? 'rgba(255, 255, 255, 0.3)' : 'rgba(0, 0, 0, 0.3)',
                  color: isDarkMode ? 'white' : '#000000',
                  '&:hover': {
                    borderColor: isDarkMode ? 'rgba(255, 255, 255, 0.5)' : 'rgba(0, 0, 0, 0.5)',
      ...
      <Dialog open={confirmDialogOpen} onClose={() => setConfirmDialogOpen(false)}>
        ...
        <DialogContent>
          <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
            ...
            <Typography sx={{ color: isDarkMode ? 'white' : '#000000' }}>Disabling this asset as collateral may put your account at risk if you have active borrows. Are you sure you want to continue?</Typography>
          </Box>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setConfirmDialogOpen(false)} variant="outlined" sx={{
            borderColor: isDarkMode ? 'rgba(255, 255, 255, 0.3)' : 'rgba(0, 0, 0, 0.3)',
            color: isDarkMode ? 'white' : '#000000',
            '&:hover': {
              borderColor: isDarkMode ? 'rgba(255, 255, 255, 0.5)' : 'rgba(0, 0, 0, 0.5)',
          ...
          <Button onClick={confirmDisable} variant="contained" color="warning" sx={{
...
const SkeletonRow: React.FC<{ isDarkMode: boolean }> = ({ isDarkMode }) => (
  <Card sx={{
    ...
    <CardContent>
      <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
        <Box sx={{ width: 36, height: 36, borderRadius: '50%', bgcolor: isDarkMode ? 'rgba(255, 255, 255, 0.2)' : 'rgba(0, 0, 0, 0.2)' }} />
        <Box sx={{ flex: 1 }}>
          <Box sx={{ height: 14, width: '40%', bgcolor: isDarkMode ? 'rgba(255, 255, 255, 0.2)' : 'rgba(0, 0, 0, 0.2)', mb: 1, borderRadius: '4px' }} />
          <Box sx={{ height: 12, width: '30%', bgcolor: isDarkMode ? 'rgba(255, 255, 255, 0.2)' : 'rgba(0, 0, 0, 0.2)', borderRadius: '4px' }} />
        </Box>
      </Box>
    </CardContent>
  </Card>
